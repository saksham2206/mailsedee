...
class AutomationList extends Model
{
    // Automation status
     ...
     */
    protected $fillable = [
        'name','customer_id', 'mail_list_id', 'status', 'data','smtp_server_id'
    ];
   ...
   public function initTrigger($subscriber, $force = false)
    ...
    {
        return $customer->AutomationList()->where('id', '<>', $this->id)->orderBy('updated_at', 'desc')->limit(50);
    }
    ...
    public function checkAutomation($id){
        $automation = Automation2::where('main_id',$id)->get();
    ...
    public function AutomationListOption($customerId=null){
    ...
    {
        return self::select('automations.*');
    }
