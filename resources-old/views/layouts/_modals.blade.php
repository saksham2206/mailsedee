		...
		<div class="modal-dialog">
			<div class="modal-content">
				<form class="confirm-link-form" onkeypress="return event.keyCode != 13;">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-mc_inline " data-dismiss="modal">{{ trans('messages.cancel') }}</button>
						<a class="btn btn-mc_primary link-confirm-button ajax_link bg_btn">{{ trans('messages.confirm') }}</a>
					</div>
				</form>
			</div>
		</div>
				...
				<form action="{{ action('MailListController@copy') }}" method="POST" class="ajax_copy_list_form form-validate-jqueryz">
					...
					<div class="modal-body">
						...
						<div class="text-right">
							<button type="submit" class="btn btn-success bg-teal-600 mr-5">{{ trans('messages.copy') }}</button>
	...
	@endif
	@if((null !== Session::get('orig_admin_id') && Auth::user()->admin) || (Acelle\Model\Setting::get("site_online") == 'false') || (
				...
				!\Auth::user()->customer->activeSendingServers()->count()) ||  ((\Acelle\Library\Tool::currentView() == 'backend') && !\Acelle\Model\SendingServer::system()->count()) ) 
			...
			<div class="modal-content">
				<form sending-text='<i class="icon-spinner10 spinner position-left"></i> {{ trans('messages.sending_please_wait') }}' action="{{ action('CampaignController@sendTestEmail') }}" method="POST" class="ajax_send_a_test_email_form form-validate-jquery">
					...
					<div class="modal-body">
						...
						<div class="text-right">
							<button type="submit" class="btn btn-success bg-teal-600 mr-5"><i class="icon-paperplane ml-5"></i> {{ trans('messages.send') }}</button>
							<button type="button" class="btn btn-default ml-0 copy-campaign-close" data-dismiss="modal">{{ trans('messages.close') }}</button>
						</div>
