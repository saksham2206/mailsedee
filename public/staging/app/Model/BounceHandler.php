        ...
        try {
            ...
            */
                ...
                $st1 = preg_replace("/\r|\n/", "", $st);
                 ...
                 $re = '`https?://.*/p/\K([^#\s]+)/open`m';
                 preg_match_all($re, $st1, $matches, PREG_SET_ORDER, 0);
                 
                 //MailLog::info('msgBody1 =>>'.json_encode($matches));
                if(count($matches)>0){
                    $messageId = StringHelper::base64UrlDecode($matches[0][1]);
                    $msgId = $messageId;
                    
                 }
                 
                 //MailLog::info('msgBody2 =>>'.$msgId);
            ...
            $trackingLog->subscriber->sendToBlacklist($bounceLog->raw, $trackingLog->customer_id);
        } catch (\Exception $ex) {
