    ...
    ****/
    ...
    protected function doExecute()
    {
        ...
        */
        ...
        if($this->options['email_uid'] == $email->uid){
    ...
    {
        $data = json_decode($this->autoTrigger->data);
        $subsData = array();
        $email_uid ="";
        //MailLog::info('getEmailData:'.json_encode($data));
        foreach ($data as $key => $value) {
            if($key > 0){
                if ($this->getId() == $value->id) {
                    if(in_array($this->autoTrigger->subscriber->email, $value->options->subscribers)){
                        $subsData[$value->options->email_uid][] = $this->autoTrigger->subscriber->email;
    ...
    {
        //MailLog::info('Subscriber : == '.json_encode($this->autoTrigger->data));
