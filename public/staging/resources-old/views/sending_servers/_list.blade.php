    ...
    >
        @foreach ($items as $key => $item)
            <tr>
                ...
                <td>
                    <select id="BounceHandler" class="form-control" onchange="changeBounce();" data-id="{{$item->uid}}">
                        <option value="">Select Bounce Handler</option>
                        @php
                        $bounceHandels = getAllBounceHandler();
                        @endphp
                        @foreach($bounceHandels as $bounceHandel)
                        <option value="{{$bounceHandel->id}}" <?php echo $bounceHandel->id == $item->bounce_handler_id ? 'selected':'';?>>{{$bounceHandel->name}}</option>
                        @endforeach
                    </select>
                </td>
                <td>
...
<script type="text/javascript">
    function changeBounce(){
        ...
        var bounceId = $(this).val();
        $.ajax({
            url : "{{url('bounce_handlers/updateServer')}}",
