            ...
            <tr>
                ...
                <td>
                    <h5 class="no-margin text-bold">
                        <a class="kq_search" href="{{ action('FeedbackLoopHandlerController@edit', $item->uid) }}">{{ $item->name }}</a>
                    </h5>
                ...
                <td class="text-right">
                    @can('update', $item)
                        <a href="{{ action('FeedbackLoopHandlerController@edit', $item->uid) }}" data-popup="tooltip" title="{{ trans('messages.edit') }}" type="button" class="btn bg-grey btn-icon"><i class="icon-pencil"></i> {{ trans('messages.edit') }}</a>
                    @endcan
                    @if(Auth::user()->admin->can('delete', $item) || Auth::user()->admin->can('test', $item))
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                @can('test', $item)
                                    <li>
                                        <a href="{{ action('FeedbackLoopHandlerController@test', $item->uid) }}" data-method="POST" type="button" class="ajax_link">
                                ...
                                @can('delete', $item)
                                    <li>
                                        <a delete-confirm="{{ trans('messages.delete_feedback_loop_handlers_confirm') }}" href="{{ action('FeedbackLoopHandlerController@delete', ["uids" => $item->uid]) }}">
