       ...
       <!--  <div class="col-md-12">
        ...
        @if (Auth::user()->customer->can("create", new Acelle\Model\EmailVerificationServer()))
            <div class="col-md-12">
                <div class="content-group-sm">
                    <div class="pull-right text-teal-800 text-semibold">
                        <span class="text-muted">{{ \Acelle\Library\Tool::format_number(Auth::user()->customer->subscription->email_verification) }}/{{ \Acelle\Library\Tool::format_number(Auth::user()->customer->maxEmailVerificationServers()) }}</span>
                        @php
                        $usedRecord = Auth::user()->customer->maxEmailVerificationServers() - Auth::user()->customer->subscription->email_verification;
                        $precentage = ($usedRecord / Auth::user()->customer->maxEmailVerificationServers())*100;
                        @endphp
                        &nbsp;&nbsp;&nbsp;{{ round($precentage,2) }}%
                    </div>
                    <h5 class="text-semibold mb-5">{{ trans('messages.email_verification_server') }}</h5>
                    <div class="progress progress-xxs">
                        <div class="progress-bar bg-warning" style="width: {{ $precentage }}%">
                        </div>
                    </div>
                </div>
            </div>
