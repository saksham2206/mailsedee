            ...
            <tr>
                ...
                <td>
                    <h5 class="no-margin text-bold">
                        <a class="kq_search" href="{{ action('BounceHandlerController@edit', $item->uid) }}">{{ $item->name }}</a>
                    </h5>
                ...
                <td class="text-right">
                    ...
                    @can('update', $item)
                        <a href="{{ action('BounceHandlerController@edit', $item->uid) }}" data-popup="tooltip" title="{{ trans('messages.edit') }}" type="button" class="btn bg-grey btn-icon"><i class="icon-pencil"></i> {{ trans('messages.edit') }}</a>
                    @endcan
                    @can('delete', $item)
                        <div class="btn-group">                                        
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">                                                    
                                <li>                                                        
                                    <a delete-confirm="{{ trans('messages.delete_bounce_handlers_confirm') }}" href="{{ action('BounceHandlerController@delete', ["uids" => $item->uid]) }}">
