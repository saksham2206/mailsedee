...
class Customer extends Model
{
    ...
    public function AutomationList(){
        return $this->hasMany('Acelle\Model\AutomationList');
    }
    ...
    {
        $user = new User();
        $resellerId = Admin::where('admin_group_id',2)->first();
        DB::transaction(function () use ($request, &$user,$resellerId) {
            // Customer
            $this->fill($request->all());
            $this->admin_id = $resellerId->id;
    ...
    public function updateCache($key = null)
    {
        ...
        $index = [
            ...
            'SendingServerSelectOptions' => function (&$customer) {
                return $customer->getSendingServerSelectOptions([], true);
            },
     ...
     * @return array
     */
    public function getSendingServerSelectOptions($options = [], $cache = false)
    {
        ...
        $result = $query->orderBy('name')->get()->map(function ($item) use ($cache) {
            return ['id' => $item->id, 'value' => $item->id, 'text' => $item->name.' ('.$item->type.')'];
        });
    ...
    //     $result = $query->orderBy('name')->get()->map(function ($item) use ($cache,$data) {
    //         $data = array(
    //             'id' => $item->id, 'value' => $item->id, 'text' => $item->name
    //         ); 
    //         return $data;
    //     });
    //     $finalData = array_merge($datas,$)
    //     dd($result);
    //     return $result;
    // }
    /**
    ...
    public function deleteAccount()
    {
        ...
        $fetch = AutomationList::where('customer_id',$this->user->id);
