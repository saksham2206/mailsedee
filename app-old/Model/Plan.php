...
class Plan extends Model
{
    ...
    public static function getAvailablePlans()
    {
        ...
        $current_plan = $customer->getCurrentSubscription();
        
        if(isset($current_plan)){
            $current_plan_price = self::where('id', '=', $current_plan->plan_id)->first()->price;

        }
        
        //dd($subscription);
        if($subscription== null || $subscription->isNew()){

            if(isset($current_plan)){
                if($current_plan_price != '0.00'){
                    return self::active()->available()->get();
                }else{
                    $plan = self::where('price','!=','0.00')->active()->available()->get();
                    return $plan;
                }
            }else{
                return self::active()->available()->get();
